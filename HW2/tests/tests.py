...
# to run one test: python -m unittest tests.rec_error
# to run all tests: python -m unittest tests

with open('C:\\Users\\Anamika Shekhar\\Desktop\\Spring22\\10707\\HW2\\S22_HW2_handout_v2\\Programming\\tests\\tests.pk',"rb") as f:
    tests = pickle.load(f)
    
    
class h_v(unittest.TestCase): #Done
    @weight(2)
    def test(self):
        # Instantiate
        np.random.seed(SEED)
...
class sample_h(unittest.TestCase): #Done
    @weight(2)
    def test(self):    
        # Instantiate
        np.random.seed(SEED)
...
class sample_v(unittest.TestCase): #Done
    @weight(2)
    def test(self):    
        # Instantiate
        np.random.seed(SEED)
...
class sample_v_injection(unittest.TestCase): #Done
    @weight(5)
    def test(self):        
        # Instantiate
        np.random.seed(SEED)
...
class gibbs_k(unittest.TestCase): #Done
    @weight(2)
    def test(self):
        # Instantiate
        np.random.seed(SEED)
...
class update(unittest.TestCase):
    ...
    def test(self):        
        ...
        # Test
        rbm.update(v)
        print("Expect",tests["update"]["hbias"].shape)

...
# to run one test: python -m unittest tests.TestConvBackward
# to run all tests: python -m unittest tests


# 0pts
class TestReLU(unittest.TestCase): #Passed
  def test(self):
    sl = cnn.ReLU()
...
# 4pts
class TestIm2Col(unittest.TestCase):#passed
    def test(self):
...
class TestConvBackward(unittest.TestCase):
    def test(self):
        ...
        conv_obj = cnn.Conv(input_shape=(3,32,32),filter_shape=(1,5,5),rand_seed=seed)
        conv_out = conv_obj.forward(test_imgs,1,2)
        #print("This is expected",tests["conv_back_w"].shape)
...
class TestMaxPoolForward(unittest.TestCase):
    def test(self):
        ...
        #Sprint("This is expected",tests["max_out"])
...
# 1pts
class TestFCWeightsBias(unittest.TestCase): #Passed
    def test(self):
...
# 1pts
class TestLinearForward(unittest.TestCase): #Passed
    def test(self):
...
# 2pts
class TestLinearBackward(unittest.TestCase):#Passed
    def test(self):
        features = tests["linear_features"]
...
# 1pts
class TestLinearUpdate(unittest.TestCase):#passed
    def test(self):
        features = tests["linear_features"]
        linear_dloss = tests["linear_dloss"]
        linear_obj = cnn.LinearLayer(256,20,rand_seed=seed)

# -*- coding: utf-8 -*-
"""
Created on Thu Feb 10 16:24:51 2022

@author: Anamika Shekhar
"""
#!/usr/bin/env python
# coding: utf-8
...
class SingleLayerMLP(Transform):##code
    ...
    def __init__(self, inp, outp, hiddenlayer=100, alpha=0.1, lr=0.01):
        ...
        self.batch_norm1=BatchNorm(hiddenlayer,0.9,lr, mm=0.9)
        self.RELU1=ReLU()
        self.LinearMap2=LinearMap(hiddenlayer,outp,alpha,lr)
        self.batch_norm2=BatchNorm(outp,0.9,lr, mm=0.9)

    def forward(self, x, train=True):
        #print(x.shape)
        a1=self.LinearMap1.forward(x)
        a1_b=self.batch_norm1.forward(a1,train)
        #print(a1.shape)
        z1=self.RELU1.forward(a1_b)
        #print(z1.shape)
        o=self.LinearMap2.forward(z1)
        o_b=self.batch_norm2.forward(o,train)
        return o_b

    def backward(self, grad_wrt_out):
        grad_batch2=self.batch_norm2.backward(grad_wrt_out)
        grad_linear2_x=self.LinearMap2.backward(grad_batch2)
        grad_relu=self.RELU1.backward(grad_linear2_x) 
        grad_batch1=self.batch_norm1.backward(grad_relu)
        grad_linear1_x=self.LinearMap1.backward(grad_batch1)
        return grad_linear1_x

    def step(self):
        #print("Step was called")
        #print("This is before",self.LinearMap2.getW()[1,:10])
        self.batch_norm2.step()
        self.LinearMap2.step()
        self.batch_norm1.step()
    ...
    def zerograd(self):
        ...
        self.batch_norm2.zerograd()
        self.LinearMap2.zerograd()
        #print(self.LinearMap2.grad_W[1,:10])
        self.batch_norm1.zerograd()
...
#%%
    np.savetxt('SLMLP_train_loss_batch.csv',np.array(train_cost_SLP),delimiter=",")
    np.savetxt('SLMLP_test_loss_batch.csv',np.array(test_cost_SLP),delimiter=",")
    np.savetxt('SLMLP_train_acc_batch.csv',np.array(train_acc_SLP),delimiter=",")
    np.savetxt('SLMLP_test_acc_batch.csv',np.array(test_acc_SLP),delimiter=",")
#%%
    from matplotlib import pyplot as plt
    
    plt.plot(range(50),train_cost_SLP,label='Train')
    plt.plot(range(50),test_cost_SLP,label='Test')
    plt.title('Single Multi-Layer Perceptron with Batch Normalization Loss vs Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Cost')
    plt.legend()
    plt.show()
#%%    
    ...
    plt.title('Single Multi-Layer Perceptron with Batch Normalization Accuracy vs Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.show()

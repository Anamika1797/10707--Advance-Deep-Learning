# -*- coding: utf-8 -*-
"""
Created on Thu Feb 10 23:02:18 2022
...
class SingleLayerMLP(Transform):##code
    ...
    def forward(self, x, train=True):
        ...
        #print(z1.shape)
        dp=self.drop1.forward(z1,train)
        o=self.LinearMap2.forward(dp)
        return o

    def backward(self, grad_wrt_out):
        grad_linear2_x=self.LinearMap2.backward(grad_wrt_out)
        grad_drop=self.drop1.backward(grad_linear2_x)
        grad_relu=self.RELU1.backward(grad_drop)
...
#%%
    np.savetxt('SLMLP_train_loss_drop.csv',np.array(train_cost_SLP),delimiter=",")
    np.savetxt('SLMLP_test_loss_drop.csv',np.array(test_cost_SLP),delimiter=",")
    np.savetxt('SLMLP_train_acc_drop.csv',np.array(train_acc_SLP),delimiter=",")
    np.savetxt('SLMLP_test_acc_drop.csv',np.array(test_acc_SLP),delimiter=",")
#%%
    from matplotlib import pyplot as plt
    
    plt.plot(range(50),train_cost_SLP,label='Train')
    plt.plot(range(50),test_cost_SLP,label='Test')
    plt.title('Single Multi-Layer Perceptron with dropout Loss vs Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Cost')
    plt.legend()
    plt.show()
#%%    
    ...
    plt.title('Single Multi-Layer Perceptron with dropout Accuracy vs Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
    plt.show()
